...
def record(request):
  if request.method == 'POST':
        # form = VoicePatternForm(request.POST, request.FILES)
        # form = VoicePatternForm()
        # form.speaker_name = request.POST['speaker_name']
  ...
  return render(request, 'books/record.html')
